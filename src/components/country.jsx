...
const Country = ({country, handleVisitedCountrie, handleVisitedFlag}) => {
    ...
    return (
        <div className={`border-2  m-4 p-4 rounded-lg  text-black flex flex-col justify-around ${visited && 'bg-green-200'}`}>
            <div>
                <img src={flags.png? flags.png: flags.svg? flags.svg: flags.alt} alt="" />
            </div>
            <h2 style={{color: visited? 'red': 'black'}} className="text-2xl">Name: {name.common? name.common : name.official}</h2>
            ...
            <button onClick={()=> {
                handleVisited()
                handleVisitedCountrie(country)
                handleVisitedFlag(country)
            }} className='btn btn-success text-white'>Visited</button>
